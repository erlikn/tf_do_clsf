...
def get_multi_logit(pose):
    logit = np.zeros(shape=[len(BIN_min), BIN_SIZE], dtype=bool)
    for labdex in range(len(BIN_min)):
        for bindex in range(BIN_SIZE+1):
            if (pose[labdex] < BIN_rng[labdex][bindex]):
                logit[labdex][bindex-1] = 1
                break
    return logit
...
#    abgxyzA2B = abgxyzB-abgxyzA # A->B
    ...
    logit = get_multi_logit(abgxyzA2B)
...
def prepare_dataset(datasetType, pclFolder, poseFolder, seqIDs, tfRecFolder):
    ...
    for i in range(len(seqIDs)):
        tMatMin.append([1000,1000,1000,1000,1000,1000])
        tMatMax.append([-1000,-1000,-1000,-1000,-1000,-1000])
        print('Processing ', seqIDs[i])
        posePath = _get_pose_path(poseFolder, seqIDs[i])
        poseFile = _get_pose_data(posePath)
        print(posePath)
        print(poseFile.shape)
        pclFolderPath = ""#_get_pcl_folder(pclFolder, seqIDs[i])
        pclFilenames = ""#_get_file_names(pclFolderPath)
        startTime = time.time()
        num_cores = multiprocessing.cpu_count() - 2
        for j in range(0,poseFile.shape[0]-1):
            tMatAB = process_dataset(startTime, durationSum, pclFolderPath, seqIDs[i], pclFilenames, poseFile, tfRecFolder, j)
            for k in range(0, len(tMatAB)):
                if (tMatAB[k] < tMatMin[i][k]):
                    tMatMin[i][k]=tMatAB[k]
                if (tMatAB[k] > tMatMax[i][k]):
                    tMatMax[i][k]=tMatAB[k]
    #print("Mins", tMatMin)
    #print("Maxs", tMatMax)
    tMatGlobMin = [1000,1000,1000,1000,1000,1000]
    tMatGlobMax = [-1000,-1000,-1000,-1000,-1000,-1000]
    for i in range(len(seqIDs)):
        for k in range(0, len(tMatGlobMin)):
            if (tMatGlobMax[k] < tMatMax[i][k]):
                tMatGlobMax[k] = tMatMax[i][k]
            if (tMatGlobMin[k] > tMatMin[i][k]):
                tMatGlobMin[k] = tMatMin[i][k]
    print("Mins", tMatGlobMin)
    print("Maxs", tMatGlobMax)
    #jsonFile = {'tMatMin': tMatMin}
    #write_json_file("tMatMin.json", tMatMin)
    #jsonFile = {'tMatMax': tMatMax}
    #write_json_file("tMatMax.json", tMatMax)
